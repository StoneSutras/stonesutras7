...
declare %private function api:iiif-image-info($path as xs:string) {
    let $request := <http:request method="GET" href="http://sutras.adw.uni-heidelberg.de:8182/iiif/3/{$path}/info.json"/>
...
declare function api:iiif-structures($catalog as element(), $canvases as map(*)*) {
    array {
        ...
        map {
            ...
            "images": [
                map {
                    ...
                    "resource": map {
                        "@id": "http://sutras.adw.uni-heidelberg.de:8182/iiif/3/" || $id || "/full/full/0/default.jpg",
                        "@type": "dctypes:Image",
                        "format": "image/jpeg",
                        "width": $info?width,
                        "height": $info?height,
                        "service": map {
                            ...
                            "@id": "http://sutras.adw.uni-heidelberg.de:8182/iiif/3/" || $id,
...
declare function api:iiif-manifest($request as map(*)) {
    let $id := $request?parameters?id
    let $catalog := collection($config:data-catalog)/id($id)
    let $canvases := api:iiif-canvases($catalog)
    return
        map {
