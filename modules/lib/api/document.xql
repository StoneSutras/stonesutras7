        ...
        if ($xml?data) then
            ...
            return
                if ($request?parameters?format = "html") then
                    ...
                    let $next := if ($view = "single") then () else $config:next-page($xml?config, $mapped, $view)
                    let $prev := if ($view = "single") then () else $config:previous-page($xml?config, $mapped, $view)
                    return
