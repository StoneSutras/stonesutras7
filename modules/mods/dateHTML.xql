...
declare function dateHTML:output-dateHTML($entry as element(mods:mods)){
    ...
    return
        ...
        <span lang="en">{($settings:SPACE),($date [@qualifier="others"]/text())}</span>
        else if ($date/@qualifier="forthcoming") then
        <span lang="en">{($settings:SPACE),($date [@qualifier="forthcoming"]/text())}</span>
        else
            if ($date/@point="start") then
            <span lang="en">{($settings:SPACE),fn:substring($date[@point="start"]/text(),1 ,4)}–{fn:substring($date[@point="end"]/text(),1 ,4)}</span> 
            else
            <span lang="en">{($settings:SPACE),fn:substring($date[1]/text(),1 ,4)}</span>        
};
...
declare function dateHTML:output-journaldateHTML($entry as element(mods:mods)){
    ...
    return
        if ($date/@qualifier="others") then
        <span lang="en">{($date [not(@*)]/text())},{($settings:SPACE),($date [@qualifier="others"]/text())}</span>
        else
            if ($date/@point="start") then
            <span lang="en">{fn:substring($date[@point="start"]/text(),1 ,4)}–{fn:substring($date[@point="end"]/text(),1 ,4)}</span> 
            else
            <span lang="en">{fn:substring($date[1]/text(),1 ,4)}</span>        
};